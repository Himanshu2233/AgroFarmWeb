...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { to: '/harvest-calendar', label: 'Harvest', icon: '🌾' },
            ...
            {user ? (
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  {user.profile_image ? (
                    <img 
                      src={`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${user.profile_image}`}
                      alt={user.name}
                    ...
                    <div className="px-4 py-3 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-100">
                      <div className="flex items-center gap-3 mb-2">
                        {user.profile_image ? (
                          ...
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-600 to-emerald-600 text-white flex items-center justify-center font-bold text-lg shadow-sm">
                            {user.name?.charAt(0).toUpperCase()}
                          </div>
                        )}
